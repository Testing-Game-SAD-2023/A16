            ...
            async function fetchTurns(turno) {
              for(var i=1; i<=turno; i++){
                ...
                try{
                  ...
                  let response = await fetch('http://localhost:3080/tests/Game'+val.toString() + '/Test' + localStorage.getItem("classe"), { method: 'GET' }); //A16 - Aggiunto percorso Game
                  if (!response.ok) {
                    throw new Error('Richiesta risultati turno non andata a buon fine');
                  }
      ...
      runButton.addEventListener("click", function() {
        ...
        $(document).ready(function() {
          if(localStorage.getItem("gameId") == "null") { //controllo game invece che turn
            ...
            formData.append("playerId", String(parseJwt(getCookie("jwt")).userId)); //A16 - Integrazione
            ...
            //Seguenti due righe aggiunte da A16
            //formData.append("order", orderTurno);
            formData.append("username", localStorage.getItem("username"));
            $.ajax({
              url: "/api/run_all", // con questa verso il task 6, si salva e conclude la partita e si decreta il vincitore
              type: "POST",
              data: formData,
              processData: false,
              contentType: false,
